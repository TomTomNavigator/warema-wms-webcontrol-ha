...
class WaremaShade(CoverEntity):
    ...
    def update(self, force=False):
        if (datetime.now() > self.next_state_update
                ...
                or force):
            was_moving = self.shade.is_moving
            ...
            if was_moving and not self.shade.is_moving:
                ...
                # Home Assistant polls every 30s. Force updates for 30s to ensure exactly one post-movement poll.
                self.force_update_until = datetime.now() + timedelta(seconds=30)
    ...
    def open_cover(self, **kwargs):
        ...
        # 52 corresponds to -75° (127 is 0°)
        tilt = 52 if self.shade.tilt is not None else None
        self.shade.set_shade_position(0, tilt)

    def close_cover(self, **kwargs):
        self.force_update_until = datetime.now() + timedelta(seconds=15)
        tilt = self.shade.tilt
        # Reset negative tilt to 0° (127) when closing
        if tilt is not None and tilt < 127:
            tilt = 127
        self.shade.set_shade_position(100, tilt)

    def set_cover_position(self, **kwargs):
        self.force_update_until = datetime.now() + timedelta(seconds=15)
        position = 100 - kwargs[ATTR_POSITION]
        tilt = self.shade.tilt
        # Reset negative tilt to 0° (127) when moving down
        if position > 0 and tilt is not None and tilt < 127:
            tilt = 127
        self.shade.set_shade_position(position, tilt)
